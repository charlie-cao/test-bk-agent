...
class ShipAnimationManager {
	constructor(audioEffects) {
		...
		this.vfx = (typeof window !== 'undefined' && window.visualEffects) ? window.visualEffects : null;
	...
	// 创建激光效果（优先使用VisualEffectsManager）
	createLaserEffect(startX, startY, targetX, targetY) {
		if (this.vfx && typeof this.vfx.createLaserBeam === 'function') {
			this.vfx.createLaserBeam(startX, startY, targetX, targetY);
