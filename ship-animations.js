...
class ShipAnimationManager {
	constructor(audioEffects) {
		...
		this.vfx = (typeof window !== 'undefined' && window.VFXManagerInstance) ? window.VFXManagerInstance : null;
	...
	// 创建激光效果（优先使用VFXManager）
	createLaserEffect(startX, startY, targetX, targetY) {
		if (this.vfx) {
			this.vfx.laser(startX, startY, targetX, targetY, '#ff6b6b');
